...
const SCHEMES = new Set<AppearanceColorScheme>(["auto", "light", "dark"]);
const HARMONIES = new Set<ThemeHarmony>([
  "complementary",
  "splitComplementary",
  "analogous",
  "triadic",
...
export function normalizeAppearancePreferences(
  ...
  const points = Array.isArray(record.points)
    ? record.points.map(normalizePoint).filter((point) => point !== null)
    : [];

  return {
    harmony: HARMONIES.has(record.harmony as ThemeHarmony)
      ? (record.harmony as ThemeHarmony)
    ...
    scheme: SCHEMES.has(record.scheme as AppearanceColorScheme)
      ? (record.scheme as AppearanceColorScheme)
...
function normalizePoint(input: unknown): ThemePoint | null {
  if (!isRecord(input)) {
    return null;
  }
