...
interface GeneratedSatelliteToolInput {
  name: string;
  description?: string;
...
function extractProposal(response: AssistantMessage): CustomSatelliteProposal {
  ...
  const generated = validateToolArguments(
    proposalTool,
    toolCalls[0]!
  ) as GeneratedSatelliteToolInput;
